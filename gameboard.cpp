...
namespace {
bool isAjacent(const GameBoard::Position f, const GameBoard::Position s){
    if (f==s){
        return false;
    }
...
bool GameBoard::move(int index) {
    ...
    if(!isAjacent(elementPosition, hiddenElementPosition)){
        return false;
    }
...
GameBoard::Position GameBoard::getRowCol(const size_t index) const {
    ...
    // проверяю не нарушен ли инвариант
...
/*
 * Метод проверяющий валидна ли сгенерированная
 * последователь плиток
*/
